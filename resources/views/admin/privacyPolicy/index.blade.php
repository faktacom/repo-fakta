...
@section('title', 'History Privacy Policy')
    ...
    <div class="card">
        <div class="card-body">
            <table class="table table-striped table-bordered" id="editable-datatable">
                ...
                <tbody>
                    @foreach ($list_privacy_policy as $item)
                    <tr id="1" class="gradeX">
                        <td>{{$loop->iteration}}</td>
                        <td>{!! $item->content !!}</td>
                        <td>{{$item->created_date}}</td>
                        <td style="text-align:center;">
                            <a href="{{route('admin.privacyPolicy.show', $item->privacy_policy_id)}}" class="btn btn-info btn-sm"><i
