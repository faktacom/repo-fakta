...
@section('title', $detailAuthor[0]->name)
...
<div class="row justify-content-center" style="grid-column-start: span 12">
    <div class="col-lg-8 p-1">
        <h4 class="fkt-tag-title text-center my-3">Berita Oleh: {{$detailAuthor[0]->name}}</h4>
        <div class="card w-100">
            <div id="draftData" class="pt-3 bg-white container fkt-search-news-container">
                @if (isset($listAuthorNews) && $listAuthorNews->count() > 0)
                    @foreach ($listAuthorNews as $news)
                ...
                @endif
                {{$listAuthorNews->onEachSide(1)->links('pagination::bootstrap-4')}}
            </div>
        </div>
    </div>
</div>
